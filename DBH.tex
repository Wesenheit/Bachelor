...
\newcommand{\summary}[1]{
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}{0.45\textwidth}
            ...
            \caption{Phased light curve in I mag from OGLE survey}
        \end{subfigure}
        %\hfill
        \begin{subfigure}{0.45\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{plots/#1_simple_corner_emcee.png}
            \caption{Corner plot for SED posterior estimate.}
        \end{subfigure}
   \end{figure}
    \begin{figure}[H]
        \centering
        \begin{subfigure}{0.45\textwidth}
            ...
            \caption{Spectral Energy Distribution plot  for single star model.}
        \end{subfigure}
        %\hfill
        \begin{subfigure}{0.45\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{plots/#1_HR.png}
            \caption{Position on HR diagram together with best fitting PARSEC evolutionary track.}
        \end{subfigure}
   \end{figure}
   \newpage
}
...
%First sample consisted of $6620$ object from Small Magellanic Cloud (SMC), Large Magellanic Cloud (LMC) and 
...
%velocity amplitude ({\it{rv\_amplitude\_robust}}) higher then $100$ km/s.
...
\citep{bressan_span_2012} evolutionary tracks. Assuming solar like metallicity simple approximate fits were obtained by choosing best fitting entry from PARSEC track using temperature and luminosity estimates from
SED fits.
\begin{table}[H]
    \footnotesize
    \centerline{
    ...
    GD2246.03.18414 & $6974^{+21}_{-28}$ & $0.806 \pm 0.008$ & $0.273$ & $0.71\pm 0.01$ & $586.3$ & $1.4$ \\[0.1cm]
    ...
    LMC606.30.48    & $5516\pm 25$ & $-0.053\pm 0.066$ & $0.047$  & $1.93^{+0.15}_{-0.13}$ & $184$ & $0.8$ \\[0.1cm]
    ...
    MBR236.09.433   & $6471\pm 10$ & $0.717\pm 0.027$ & $-$  & $2.12\pm 0.07$ & $592.1$ & $1.4$ \\[0.1cm]
    SMC711.22.1068  & $6771\pm 5$ & $0.852^{+0.017}_{-0.015}$ & $0.021$  & $1.52\pm 0.03$ & $707.4$ & $1.4$ \\[0.1cm]
    ...
    \caption{Estimated physical parameters of objects using single star model together with PARSEC mass estimates and extinction estimates.}\label{objects_fit}
\end{table}
...
and only two entries (BLG931.27.36745 and GD1097.20.23000) cannot be safely assumed to be variable in radial velocity as they weren't observed 
enough times.
\begin{table}[H]
    \footnotesize
    \centering
...
%    GD1549.19.348   & $2.8224$   & $36.72$ & $21$ & $8.64$  & $44.67$   & $0.026$ &\\
    \hline
    \hline
    GD2246.03.18414 & $0.4270$ & $27.68$ & $25$ & $23.03$ & $129.93$ & $0.097$  & $0.77$ \\[0.1cm]
...
\end{figure}
    %
%\begin{figure}[t]%    \includegraphics[scale=0.5]{plots/GD1070.18.22288/lc_comparsion.png}
%    \caption{Light curves of GD1070.18.2288 in $4$ different bands}\label{comp}
%\end{figure}
%\begin{figure}[H]
...
Interestingly assumed distance of $3$ kpc based on radio observations is nearly $3$ times higher then one obtained using parallax from Gaia DR3. %If emission really originates from the system 
...
\hspace{1cm} This line was further investigated and each object was fitted using 
...
Two masses together with inclination $i$ and radius of primary star $R_1$ form four parameters that are fitted to minimize $\chi^2$ using Nelder-Mead algorithm.
Second radius is no longer free parameter and it's constrianed by common envelope. 
...
\begin{table}[H]
    ...
    normalized $\chi^2$ values.}\label{light_curve_fits}
    \end{center}
\end{table}
One example light curve fit for object BLG931.27.36745 can be seen on the plot \ref{lc_plot}.
Few of the light curves can be well fitted using contact light curve model like BLG931.27.36745 or BLG986.08.7 where
...
\begin{appendices}
    \chapter{Spectral Energy Distribution plots}
    In the following chapter all 
    \newgeometry{margin=0mm,top=10mm}
    \summary{BLG986.08.7}
...
\end{equation}
where $X_i$ is sample from $\mathcal{U}(0,1)$. This allows to determine, that after writing estimator of $\sigma^2$ one obtain 
\begin{equation}
    \sigma^2=\frac{1}{n}\sum_i (\overline{V}-V_i)^2\approx \frac{K^2}{N}\sum \cos^2{(2\pi X_i)}=\frac{K^2}{N}\sum_i \frac{\cos{(4\pi X_i)}+1}{2}
\end{equation}
where  simplification $\overline{V}\approx v_0$ is used. Now one would like to obtain some statistical properties of this variance estimator such as expected value and variance.
It can be clearly seen that $\mathbb{E}\cos{(4\pi X_i)}=0$ while $\Var(\cos{4\pi X_i})=\frac{1}{8}$. While first value is straightforward as mean of cosine is equal to zero second one 
is more complicated but follows from the fact that $\cos{(4\pi X_i)}$ is arcsine distributed and it's variance it's well known. Hence 
\begin{align}\label{semi}
    \mathbb{E} \sigma^2 = \frac{K^2}{2N}\cdot N = \frac{K^2}{2}\\ 
    \Var (\sigma^2) = \frac{K^4}{32N^2}\cdot N = \frac{K^4}{32N}
\end{align} 
Those two values allow us to determine quality of estimator, using Central Limit Theory one can also approximate that 
\begin{equation}
