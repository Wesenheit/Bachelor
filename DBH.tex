...
\begin{abstract}
    Analysis of light curves from OGLE data was performed with the goal to look for objects with possible black hole companions 
...
success achieved in the field of X-ray based observations. Although such observations can be a great tool for black hole search, they are quite limited,
as the formation of such systems is rare \citep{portegies_zwart_formation_1997}. Since the beginning of LIGO/VIRGO observations of merging black holes,
...
Of all those sources, only around $\sim 20$ are dynamically confirmed (from dynamically confirmed binaries only Cygnus X-1 is persistent). 
...
In recent years new stellar mass BH candidates have been found due to new methods, not based on X-ray emission. New generations of sky surveys
...
Compared to X-ray observations, astrometric measurements allow one to estimate the parameters of objects with much better accuracy, as they allow to infer an inclination of a binary. 
...
is microlensing. As the massive object passes in front of a source, magnified images of the source object are created; the total observed luminosity of such object will
...
In recent years, a new approach was proposed to find objects that almost fill Roche lobes. One can expect that the light curve of such object
...
\begin{align}\label{MN93}
    ...
    \end{split}
\end{align}
where $\varphi$ denotes the relative phase, $i$ the orbital inclination, $L_0$ is the luminosity {{\it without tidal disruption}}, $\Delta L$ denotes the change of the luminosity 
...
\begin{align}
    ...
    C_3(q,f,i)=1+\frac{(1+0.0698q\sin^2{i})f^6+0.2075f^2}{(2.0223+0.3880\ln{q})f+\sin^4{i}}
\end{align}
...
solely represents the effects of temperature, gravity, and chemical composition, making this parameter the most crucial feature of the model, as it encapsulates all unknown parameters. 
...
\citet{claret_new_2000} is presented in Figure \ref{claret}.
...
Detailed positions of objects from the original samples in the sky are presented in Figure \ref{map}. Then for each entry the nearest Gaia source
...
diagram using the Gaia data was constructed and is presented in Figure \ref{HR_SMC} (with a division between SMC and LMC). Similarly, a color-magnitude diagram was constructed for
objects with statistically significant parallax and presented in Figure \ref{HR_galactic}.
Most of the objects in the samples have a relatively short period ($<3$ d); a detailed period distribution
is presented in Figure \ref{periods}.
\begin{figure}
    \begin{center}
...
Each object was analysed using the ANOVA (analysis of variance) method \citep{schwarzenberg-czerny_advantage_1989} to determine the period (denoted herafter as $P$). 
...
$50$ d. According to the main assumption in the analysis, objects should be composed of stars nearly filling their Roche lobe, so a rather short period is suggested. The exact value of the threshold can be debated; here it is used as a means to
...
parameter was either a parallax (if one was measured) or a distance. In the case of the double-star model, there were two additional parameters describing the
...
Then the theoretically calculated stellar spectrum is processed using the \texttt{pyphot}\footnote{https://github.com/mfouesneau/pyphot} 
Python library, yielding a magnitude in a required filter.

In order to find a set of best-fitting parameters, a Bayesian approach was adopted. One can denote a set of observed magnitudes as $\tilde{m}_i$, theoretically predicted
magnitudes as $m_i$, and errors of magnitudes as $\sigma_i$. One can denote by $\mathcal{U}(a,b)$ a uniform distribution with the support in the form of an interval $[a,b]$ and $\mathcal{N}(\mu,\sigma^2)$ as a
...
The following catalogues were used to assemble SEDs:
\begin{enumerate}
...
\end{itemize} 
\end{enumerate}
In the case of Magellanic Clouds, extinction estimates were based on the map \citet{skowron_ogle-ing_2021}, while in the case of Galactic Disc extinction
was obtained using the Python library \texttt{ mwdust} \citep{bovy_galactic_2016} with the 3D dust map being a combination of \citet{green_3d_2019}, \citet{marshall_modelling_2006},
...
Chain convergence was checked with a corner plot, two example plots are presented in Figure
\ref{corner_1}.
...
If the BIC score was lower in the case of the single star model, then the star was selected as a candidate.
If the BIC score was lower in the case of the double star model, then the object was rejected unless
\begin{itemize*}
    \item the most luminous star in the double star model was significantly brighter than second one and had $\log g=2$,
    \item some stars in the model had unphysical solutions (i.e. luminosity was too low for star with $T=20000$ K).
\end{itemize*}
...
Even if some objects will not be accurately described, it is still possible to reliably assess if there are one or more stars contributing to the spectra.
The SED fits, together with the light curves are presented in the Appendix A.
%Each subsection consists of the phased light curve, the
...
The mass of every single object was estimated using the PARSEC
...
Each model was fitted with a track, using the nearest metallicity from PARSEC data ($Z=0.014$ in the case of objects inside the Galactic Disc and $Z=0.004$ in the case of the SMC).
...
In this section, a detailed characterization of individual objects is provided. It is divided into $3$ subsections, where the first two briefly cover the information
available for objects GD1070.18.22288 and SMC720.28.40576 while the third is dedicated to the remaining objects.
...
Due to an excessive amount of observations in many parts of the spectra, it was possible to obtain a high-quality spectral energy distribution fit that revealed two sources with different temperatures;
detailed distribution can be seen in Figure \ref{GD1070SED}.
One of the objects with a rather low effective temperature of
$T\approx 3800$ K has radius $R\approx 10$ $\textrm{R}_{\odot}$ and is quite far from any evolutionary track in the PARSEC database. This can indicate
that has been stripped in the past by its companion.
The position of the cold companions in the HR diagram is presented in Figure \ref{HR_cold}.
...
as a rotational variable star with half of the period found in this work. The ASAS-SN observed it in the $V$ and SDSS $g$ bands, allowing light curves to be compared with the OGLE curve.
...
The XMM X-ray source was observed a few times and is transient in nature, changing its flux slightly across time.
...
\hspace*{17.62482 pt} As one can see in Figure \ref{comp} there is a discrepancy between the curves in the $I$ band and the curves from ASAS-SN.
...
there is a visible change around $2459100$ HJD when the period decreases from $\sim 45$ d to nearly half the value around $\sim22.4$ d.
This observation, together with the evolution of the amplitude of the variability as presented in Figure \ref{evolution},
...
The most plausible explanation states that the system is similar to the RS Canum Venaticorum variable, which has undergone mass transfer in the past.
...
However, since the object does not represent a class of ellipsoidal variables, this line of investigation is dropped as it is beyond the 
scope of the work.
...
The detailed light curve together with the residuals of the fourth-degree Fourier model is presented in Figure \ref{SMC720:lc}.
%Estimated parameters suggest it isspectral type O according to a single-model SED fit \ref{SMC720:sed}.
The SED fit estimate suggests the star to be spectral type O. The photometric luminosity measured in each band together with the best-fitting spectra are presented in figure \ref{SMC720:sed}.
The PARSEC evolutionary track estimates the mass of the object to be around $16$  $\textrm{M}_{\odot}$.
The star was published already in \citet{pawlak_ogle_2016} and classified as a contact binary. Although
no companion is visible in the SED distribution, it might hide itself in the light of a primary star.
No WISE source could be found, as the nearest detection is nearly $10$" away from the position; the lack of any observations
...
If for some set of parameters, the predicted probability is very low
such a region shouldn't be populated by samples, resulting in a uniformly populated region, where a possible companion star can reside.
...
database\footnote{https://www.astronexus.com/hyg} and are presented in Figure \ref{posterior}
\begin{figure}[H]
...
The majority of objects in the list can be characterised as objects with intermediate temperatures. They typically have a radius of
...
As a part of Gaia preprocessing, the light curve is matched with the precomputed set, then the best model serves as a starting point in the local optimiser trying to refine the solution further.
...
temperature difference (ratio close to $1$). This result points out that it is possible to obtain similar results
with the contact system of two similar stars. In such a compact setup stars are in thermal equilibrium so 
the SED incorrectly fits them with the single star model.
%they can fool the SED as
...
PHOEBE simulation software \citep{wilson_realization_1971,prsa_computational_2005,conroy_physics_2020}.
...
Only objects from the Galactic Disc sample were analysed, as for them it is possible to compare estimates of radial velocity semiamplitude with
...
\begin{table}[H]
    ...
    \end{center}
\end{table}
Some of the light curves can be well fitted by using the contact light curve model. Objects BLG931.27.36745 or BLG986.08.7 not only
...
In some cases such as GD1097.20.2300 or GD1448.27.17, there is some room for improvement; there are discrepancies between the model
...
results, the predicted value of velocity semi-amplitude is twice that of the value estimated using Gaia data.
Moreover, the amplitude of radial velocity measured by the Gaia mission is equal $ 410.92 $ km/s questioning the previous estimate of radial velocity.
...
If this value is correct and if one assumes the mass of the primary star from the PARSEC fit, the lower bound for the companions mass is $\sim 0.8$ M$_\odot$, detailed 
dependence between $\sin(i)$ and $M_2$ is presented in Figure \ref{second_mass}. Hence, GD2246.03.1814 is potentially the most promising candidate for a 
dormant compact companion. All light curve models are presented in Appendix C.
...
Despite this fact, as demonstrated, not only some of the objects can be well described using the contact model, but in all cases contact model can achieve an amplitude comparable to the observed one. This makes such binaries one of the great weaknesses of the method described
...
This is not unexpected as after limiting $I<17$ mag, only the most luminous objects can pass this threshold.
...
due to the great amounts of contact binaries in the universe, contact scenarios seem to be much more plausible.
Gaia DR3 semiamplitude estimations suggest that most of the companions are not compact at all. Even for stars with the estimated radial velocity amplitude $\sim100$ km/s
the companion's mass lower bound is too low for a compact star.
...
it was found that at least some of them would be detected.
...
\begin{appendices}
    ...
    For each object, either the nearest evolutionary track is plotted, or, in the case of objects that could not be well fitted, some nearest evolutionary tracks are plotted for comparison.
    \newpage
    \summary{BLG986.08.7}
    ...
    \begin{align}
        ...
        \end{split}
    \end{align}
    where it was used used, that the expected value of the sinusoidal signal is zero.
    Hence 
    \begin{equation}
